...
protocol EventUpdatesDelegate: AnyObject {
  func didChangeEvent(_ dict: Payload,
...
final class ActivityService: AbstractService {
  ...
  func startService() throws -> ServiceError? {
    logActivity.log("Starting activity service")
    if isAuthorized() {
      logActivity.log("Permissions granted, proceed to start activity service")
      guard !isRunning else {
        logActivity
        .error("Failed to start activity service, the service is already running.")
        return nil
      }
      repeatTimer?.suspend()
      isRunning = true
      ...
      logActivity
        .error("Failed to start activity service because permissions \(accessLevel)")
  ...
  private func startTrackingStep() {
    logActivity.log("Starting pedometer updates")
    ...
    if CMPedometer.isStepCountingAvailable() {
      pedometer.startUpdates(
        ...
        withHandler: { data, error in
          ...
          if error == nil {
            if let lastPedometerData = self.lastPedometerData,
              let data = data,
              lastPedometerData.numberOfSteps.intValue == data.numberOfSteps
              .intValue {
            } else {
              self.lastPedometerData = data
            }
  ...
  fileprivate func handleActivityReceived(activity: CMMotionActivity) {
    ...
    if activity.confidence == .high,
      ...
      logActivity.log(
        "Handled activity event after checking condition: \(event)"
      )
      ...
      eventBus?.post(
        ...
        userInfo: [
          Constant.Notification.Activity.ActivityChangedEvent.key: event
        ]
      )
    }
  ...
  fileprivate func checkActivityPermission() {
    if accessLevel != currentPermissionState {
      ...
      eventBus?.post(
        name: Constant.Notification.Activity.PermissionChangedEvent.name,
        userInfo: [
          Constant.Notification.Activity.PermissionChangedEvent.key: accessLevel
        ]
      )
      guard let currentPermissionState = self.currentPermissionState else {
        return
