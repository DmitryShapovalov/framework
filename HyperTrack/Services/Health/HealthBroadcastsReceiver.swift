...
AbstractHealthBroadcastsReceiver {
  ...
  private func addObserverForTracking() {
    ...
    eventBus?.addObserver(
      ...
      name: Notification.Name.NSProcessInfoPowerStateDidChange.rawValue
    )
  }
  ...
  @objc private func stopTrackingEvent(_ notif: Notification) {
    ...
    guard let input = getTypeFromTrackingReason(reason: trackingReason)
      else { return }
  ...
  @objc private func locationPermissionEvent(_ notif: Notification) {
    ...
    guard
      let input = getTypeFromLocationPermission(accessLevel: accessLevel)
      else { return }
    checkDataStoreSystemUpdate(input: input, date: Date())
  }
  ...
  @objc private func activityPermissionEvent(_ notif: Notification) {
    ...
    guard
      let input = getTypeFromActivityPermission(accessLevel: accessLevel)
      else { return }
    checkDataStoreSystemUpdate(input: input, date: Date())
  }
  ...
  private func checkDataStoreSystemUpdate(input: HealthType, date: Date) {
    ...
    switch (
      ...
      case ((_, .resumed, .granted, .notTracking), .trackingStopped):
        updateHealthValue(input: input, date: date)
      case (
      (resumptionKey, .resumed, .denied, .notTracking),
        .permissionDenied
      ):
        updateHealthValue(input: input, date: date)
      default: break
...
extension HealthBroadcastsReceiver {
  ...
  private func getTypeFromTrackingReason(
  ...
  private func updateHealthValue(input: HealthType?, date: Date) {
    ...
    if savedValue != event.healthValues().key {
      ...
      let updatedEventDate = inputDate.addingTimeInterval(1.0)
      updatesDelegate?.updateHealth(
        event: HealthServiceData(healthType: event, recordedDate: updatedEventDate)
      )
    }
  }
}
...
extension HealthBroadcastsReceiver {
  ...
  @objc private func backgroundRefreshStatusDidChangeEvent() {
    ...
    logLifecycle
      .log("Application Background Refresh Status Did Change on: \(status)")
  }

  // low power mode change notification
  @objc private func powerStateChanged() {
    let lowerPowerEnabled = ProcessInfo.processInfo.isLowPowerModeEnabled
    logLifecycle.log("Low power mode did change to: \(lowerPowerEnabled)")
  }
}
...
extension HealthBroadcastsReceiver {
  ...
  private func updateServiceOutageGenerateHealthValue(
    ...
    if savedValue != event.healthValues().key {
      ...
      updatesDelegate?.updateHealth(
        event: HealthServiceData(healthType: event, recordedDate: date.addingTimeInterval(1.0))
      )
    }
  }
}
