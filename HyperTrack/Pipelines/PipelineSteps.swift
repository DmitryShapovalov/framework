...
protocol PipelineLogging: AnyObject {}
...
>, TransmissionDatabaseStepInput {
  ...
  override func execute(input: Pipeline.Transmission.Input.ReadDatabase)
    -> Task<Pipeline.Transmission.Input.Mapper> {
      setState(.executing)
      let taskSource =
        TaskCompletionSource<Pipeline.Transmission.Input.Mapper>()
...
final class DeivceSettingsStep:
  AbstractPipelineStep<Void, Response> {
  weak var apiClient: AbstractAPIClient?

  init(input: AbstractAPIClient?) {
    apiClient = input
    super.init()
  }
  ...
  override func execute(input _: Void) -> Task<Response> {
    ...
    apiClient.makeRequest(ApiRouter.deviceSettings)
      .continueWith(
        continuation: { [weak self] (task) -> Void in
...
final class AuthorizationTokenCheckStep: AbstractPipelineStep<Void, Bool> {
  ...
  override func execute(input _: Void) -> Task<Bool> {
    ...
    let token = reauthStep.tokenProvider?.authToken?.token ?? ""
    let tokenExpTimeInterval = reauthStep.tokenProvider?.authToken?
      .expiresIn ?? 0
    let tokenExpireTime = Date(timeIntervalSince1970: tokenExpTimeInterval)
    let isTokenTimeExpired = currentTime > tokenExpireTime
    let isAuthRequestAlreadySent = reauthStep.tokenProvider?.status == .pending

    logAuth.log(
      "Token: \(token.isEmpty ? "Empty" : token), currentDate: \(currentTime),  token expire time: \(tokenExpireTime), is token time expired: \(isTokenTimeExpired), is auth request already sent: \(isAuthRequestAlreadySent) "
    )

    if isAuthRequestAlreadySent {
      reauthStep.tokenProvider?.pendingTokenCallBack = { status in
        if taskSource.task.completed { return }
        if status == .active {
          taskSource.set(result: true)
          self.setState(.success)
    ...
    } else if !token.isEmpty, !isTokenTimeExpired {
      taskSource.set(result: true)
      setState(.success)
    } else {
      reauthStep.execute(input: ())
...
ReAuthorizeStepInput {
  ...
  override func execute(input _: Void) -> Task<Response> {
    ...
    tokenProvider?.status = .pending
    apiClient.makeRequest(ApiRouter.getToken(deviceId))
      .continueWith(
        continuation: { [weak self] (task) -> Void in
          guard let self = self else { return }
          if let error = task.error {
            ...
            do {
              let auth = try JSONDecoder.hyperTrackDecoder.decode(
                AuthToken.self,
                from: data
              )
              self.tokenProvider?.authToken = AuthToken(
                token: auth.token,
                expiresIn: currentTime.timeIntervalSince1970 + auth.expiresIn
              )
