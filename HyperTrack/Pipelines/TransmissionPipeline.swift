...
protocol AbstractTransmissionPipeline: AbstractPipeline {
  ...
  var registrationStep: AbstractPipelineStep<Void, Bool> { get }
  var tokenAuthStep: AbstractPipelineStep<Void, Bool> { get }
    ...
    reachabilityStep.execute(input: ()).continueWithTask(
      ...
      continuation: { [unowned self] (task) -> Task<Bool> in
        switch task.mapTaskToResult() {
          case .success: return self.tokenAuthStep.execute(input: ())
      ...
      continuation: { [unowned self] (task) -> Task<Bool> in
        switch task.mapTaskToResult() {
          case .success: return self.registrationStep.execute(input: ())
          case let .failure(error):
            completionHandler?(error)
            throw error
        }
...
final class TransmissionPipeline {
  ...
  let registrationStep: AbstractPipelineStep<Void, Bool>
  let tokenAuthStep: AbstractPipelineStep<Void, Bool>
  ...
  init(input: Input) {
    ...
    registrationStep = InitDeviceRegistrationEntity(
      apiClient: input.apiClient,
      appState: input.appState
    )
    tokenAuthStep = AuthorizationTokenCheckStep(
      input: ReAuthorizeStep(
        input: Initialization.Input.ReAuthorize(
          tokenProvider: input.tokenProvider,
          apiClient: input.apiClient,
          detailsProvider: input.appState
        )
      )
    )
