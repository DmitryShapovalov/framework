...
extension Log {
  ...
  func error(
    ...
    function: String = #function,
    line: Int = #line
  ) { return error(logString, file: file, function: function, line: line) }
  ...
  func fault(
    ...
    function: String = #function,
    line: Int = #line
  ) { return fault(logString, file: file, function: function, line: line) }
}

struct LogStub: Log {
  func log(_: String, file _: String, function _: String, line _: Int) {}
  func info(_: String, file _: String, function _: String, line _: Int) {}
  func trace(file _: String, function _: String, line _: Int) {}
  ...
  func fault(_: String, file _: String, function _: String, line _: Int) {}
}

@available(iOS 10.0, *) struct Logger: Log {
  let osLog: OSLog
...
func headerReduer(sum: String, header: (key: AnyHashable, value: Any)) -> String
...
func prettyPrintURLRequest(_ request: URLRequest?) -> String {
  switch request {
    ...
    case let .some(urlRequest):
      ...
      let httpMethod = urlRequest.httpMethod ?? "UNKNOWN HTTP METHOD"
      let absoluteString = urlRequest.url?.absoluteString ?? "unknown.url"
      let string = """

      \(httpMethod) \(absoluteString)
      \("Headers: \(httpHeaders)")
      \(body)
...
@available(iOS 10.0, *) extension OSLog {
  @inlinable func log(
    _ value: String,
    file: String,
    function: String,
  ...
  @inlinable func trace(file: String, function: String, line: Int) {
    #if targetEnvironment(simulator)
      // @workaround for simulator bug in Xcode 10.2 and earlier:
      // https://forums.developer.apple.com/thread/82736#348090
      let type = OSLogType.default
  ...
  @usableFromInline func _log(
    ...
    let filename = file.split(separator: "/").last.flatMap { String($0) }
      ?? file
    os_log(
