...
AbstractServicesConfig, AbstractCollectionConfig
{}
...
extension Config {
  struct Services: Codable {
    var types: [Constant.Config.Services.ServiceType]

    init() {
      types = Constant.Config.Services.types
    }
    ...
    init(from decoder: Decoder) throws {
      let container = try decoder.container(keyedBy: Keys.self)
      types = (try? container
        .decode([Constant.Config.Services.ServiceType].self, forKey: .types))
        ?? Constant.Config.Services.types
    }
