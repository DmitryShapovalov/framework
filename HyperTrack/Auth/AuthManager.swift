...
protocol AuthTokenProvider: AnyObject {
  ...
  var pendingTokenCallBack: ((AuthManager.Status) -> Void)? { get set }
...
final class AuthManager: AuthTokenProvider {
  ...
  var pendingTokenCallBack: ((AuthManager.Status) -> Void)?
  ...
  func eraseAuthData() {
    ...
    dataBase?.getDatabaseManager(.online)?.deleteAll(result: {
      transactionState in
      if transactionState {
        logAuth.log("Removed all items from online database table")
      } else {
        logAuth.error(
    ...
    dataBase?.getDatabaseManager(.custom)?.deleteAll(result: {
      transactionState in
      if transactionState {
        logAuth.log("Removed all items from custom database table")
      } else {
        logAuth.error(
